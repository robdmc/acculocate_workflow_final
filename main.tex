...
Having chosen to model the cable and background signals as Gamma distributions, we will use Monte-Carlo simulation (see Algorithm \ref{monte_algo}) to randomly draw values from our model distributions and propagate those draws through the algebra required to compute the corresponding bearing-correction angle for each draw \footnote{
    There is a detail in Algorithm \ref{monte_algo} is not readily apparent and deserves a short explanation.  It is important to ensure that the random values we draw for $m$, $s$, and $b$ are able to form a valid triangle. We use the law of cosines, equation \ref{eq:law_of_cos},  and the fact that $\left|\cos\theta\right| <=1$ to derive the following constraint
    \begin{equation} \label{eq:bounds}
          \left(m-s\right)^2 \leq b ^ 2 \leq \left(m + s\right)^2.  
    \end{equation}
    
}.
In this way, we can build of a statistical distribution capable of placing probabilistic constraints on the bearing-correction angle.
Fig. \ref{fig:correction_hist} shows such a histogram of our statistically modeled bearing-angle correction.  
...
\begin{figure}[H]
  \caption{
  The empirical distribution function (ECDF) for the same samples shown in Figure \ref{fig:correction_hist}.  ECDF plots are useful for identifying confidence intervals.  The statistical model provides  $95\%$ confidence that the bearing correction angle will fall within $\pm 37\degree$, and  $66\%$ confidence that it will fall within $\pm 28\degree$.
  }
...
We chose to implement Algorithm \ref{monte_algo} using the Python language.  Python offers excellent support for data-processing, statistics, and visualization.  We employed the widely used array-processing and mathematics libraries \texttt{numpy} and \texttt{scipy}, which, due to their array-processing paradigm,  cause the implementation to look quite different from the algorithm pseudo-code.  For completeness, this section contains our Python implementation of the sampling algorithm.
